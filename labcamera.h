...

{
	...
	public:
		...
		void Set(const glm::vec3 &position, const glm::vec3 &center, const glm::vec3 &up)
		{
			...
			distanceToTarget = sqrt((center.x - position.x)*(center.x - position.x) + (center.y - position.y)*(center.y - position.y));
		}
		...
		//nu modific right pt ca se face rotatia in jurul lui
		void RotateFirstPerson_OX(float angle)
		{
			...
			// Use glm::rotate()
			// dreapta-stanga
			forward = normalize(vec3(rotate(mat4(1), angle, right) * vec4(forward, 1)));
		...
		void RotateFirstPerson_OY(float angle)
		{
			...
			forward = normalize(vec3(rotate(mat4(1), angle, vec3(0, 1, 0)) * vec4(forward,1)));
			right = normalize(vec3(rotate(mat4(1), angle, vec3(0, 1, 0)) * vec4(right, 1)));
		...
		void RotateFirstPerson_OZ(float angle)
		{
			...
			right = normalize(vec3(rotate(mat4(1), angle, forward) * vec4(right, 1)));
			up = cross(right, forward);
		}
